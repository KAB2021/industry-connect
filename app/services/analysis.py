...
def run_analysis(session_factory: sessionmaker) -> list[AnalysisResult]:
    """Analyse all unanalysed OperationalRecords.

    Parameters
    ----------
    session_factory:
        A SQLAlchemy :class:`~sqlalchemy.orm.sessionmaker` used to obtain a
        database session. The function manages its own session lifecycle.
    ...
    try:
        ...
        if total_tokens <= settings.TOKEN_THRESHOLD:
            ...
            while reduce_tokens > settings.TOKEN_THRESHOLD:
                # Recursive reduce: summarise the summaries
